...
var respondents = mysqlTable("respondents", {
  id: int("id").autoincrement().primaryKey(),
  // Demographics
  ...
  indexAversion: int("index_aversion").default(0),
  // Store as integer (0-700 for scale 1-7 * 100)
...
async function getAnalysisData() {
  const db = await getDb();
  if (!db) return null;
  const data = await db.select().from(respondents);
  return data;
}
...
async function getSegmentationAnalysis() {
  const db = await getDb();
  if (!db) return null;
  const data = await db.select().from(respondents);
  const highIntention = data.filter((r) => (r.indexIntention || 0) >= 500);
  const lowIntention = data.filter((r) => (r.indexIntention || 0) < 500);
  return {
    highIntention: {
      count: highIntention.length,
      avgConfidence: highIntention.reduce((sum, r) => sum + (r.indexConfidence || 0), 0) / highIntention.length / 100,
      avgSecurity: highIntention.reduce((sum, r) => sum + (r.indexSecurity || 0), 0) / highIntention.length / 100,
      avgCommunication: highIntention.reduce((sum, r) => sum + (r.indexCommunication || 0), 0) / highIntention.length / 100
    },
    lowIntention: {
      count: lowIntention.length,
      avgConfidence: lowIntention.reduce((sum, r) => sum + (r.indexConfidence || 0), 0) / lowIntention.length / 100,
      avgSecurity: lowIntention.reduce((sum, r) => sum + (r.indexSecurity || 0), 0) / lowIntention.length / 100,
      avgCommunication: lowIntention.reduce((sum, r) => sum + (r.indexCommunication || 0), 0) / lowIntention.length / 100
    }
  };
}
